...
export function RelaySocket({
  ...
  return (
    <div className="rounded-sm border border-[#c9c2b4] bg-[#f4f0e6] p-2 text-[#1a1814]">
      <p className="mb-1 font-mono text-[10px] tracking-widest uppercase opacity-70">
        Mating end · 4-3 / 1-2 like the book
      </p>
      <div className="mx-auto grid w-36 grid-cols-2 gap-1 rounded-xs border-2 border-[#1a1814] bg-[#f4f0e6] p-1.5">
        {cells.map((c) => {
          const p = by[c.id];
          const on = pinId === c.id;
          return (
      ...
      <p className="mt-1.5 text-xs">Small black cube. Four square holes. Numbers 4 and 3 on top, 1 and 2 on the bottom.</p>
    </div>
  );
}
