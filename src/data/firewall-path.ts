...
const ENG_RE =
  /cec|379|starter|crank motor|crank motor solenoid|j31|glow plug|alternator|generator|pump motor|water probe|vacuum switch|fuel heater|on the engine|on trans|speedometer sensor|transmission oil temp \(345\)|fuel sender|headlight \(|horn 605|shutter solenoid|fog light \(|left turn|right turn|ether|662|396|batteries|dual mounted|camshaft|bap |filter \(399\)|inline \(401\)|booster|flow switch|diff/i;
...
export function inferZone(n: { id: string; label?: string; kind?: string; sub?: string }): WallZone {
  const t = `${n.id} ${n.label ?? ""} ${n.sub ?? ""} ${n.kind ?? ""}`;
