# 3600 Wiring

The app for a **Thomas / International 3600** with a **Navistar T444E 7.3**. Official CTS-5123V names. Hydraulic brakes.

This is an **app**, same idea as Project Cost Tracker — not a website.

## Install

**[Open the app](https://thelubemaster.github.io/thomasvista3600/?install=1)**

| Phone | How |
| --- | --- |
| **Android** | Download the app → Install → open **3600 Wiring** |
| **iPhone** | Safari → Share → Add to Home Screen |

Direct package: [3600-wiring.apk](https://thelubemaster.github.io/thomasvista3600/3600-wiring.apk)

Once it’s on the home screen, updates check themselves when you open it.

## What’s in the app

**Dx** — garage trees from a real no-crank job: 387 pin 85/86, thermal overcrank splice, magnetic switch J30 vs starter J31, CPS / tach 0.

Fuse panel, circuits (engine-critical vs not), relay pinouts, firewall cavities, hop paths, 3D shop, job keep/cut marks.

Ctrl+K search understands “no crank”, “pin 85”, “CPS”.

Job marks stay on the phone.

## 1.2.3

- Circuit drawings: cab (warm) vs engine (cool), circuit numbers on the wires, hop list “399 → plug 2 → 401 → probe”. Tap a line to light that path.

## 1.2.2

- Every circuit drawing: if a wire crosses the firewall it lands on DASH CONNECTOR (2), ENGINE DASH (3), FRONT END (2B), or a labeled dedicated pass (17B 10PK).

## 1.2.1

- Circuit 19: WATER-IN-FUEL MODULE (470) stays in the cab. Probe (19B) and lamp test (19C / 1CR) splice at FUEL FILTER (399). Only 19A/B/C cross DASH CONNECTOR (2) H5/E4/H6.

## 1.2.0

- Diagnose tab (no crank / click no crank / tach 0 / has RPM no fire)
- START RELAY (387) ISO 86 = coil + (key START), 85 = constant ground through thermal overcrank
- Magnetic switch J30 (engine-top green rings) vs J31 on the starter
- CPS on the front cover in the shop
- Circuit / book search
- APK versionName follows `VERSION`
